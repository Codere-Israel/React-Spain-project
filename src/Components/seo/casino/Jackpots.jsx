...
const Jackpots = (props) => {
  const table_of_contents_list = [
    ...
    {
      title:
        "Tu oportunidad para llevarte el bote acumulado la tienes todos los días",
      id: "leva",
    },
    {
  ...
  return (
    <>
      <Helmet>
        ...
        />
        <link rel="canonical" href="https://www.codere.es/casino/jackpots" />
      </Helmet>
      {/* End of SEO block */}

      <Floating text="¡Oportunidades que Molan!" juega={true} />
      <div
      ...
      <Container>
        ...
        <p>
          La posibilidad de ganar un Jackpot puede aumentar la emoción y la
          diversión al jugar <Link to="/casino"> casino en línea</Link>. Además,
          puedes apostar desde la palma de tu mano en cualquier momento
        </p>
        ...
        <div id={juega.id} className="mt-4">
          <h2>{juega.h2}</h2>
          <p>
            Te preguntarás: ¿cuáles son las{" "}
            <Link to="/casino/slots">slots</Link> de Codere donde puedo
            encontrar los Jackpots? Pon mucha atención y no olvides estos
            juegos:
          </p>
        ...
        <div id={tipos.id} className="mt-4">
          <h2>{tipos.h2}</h2>
          <p>{tipos.text}</p>
        ...
        <div id={grande.id} className="mt-4">
          <h2>{grande.h2}</h2>
          {grande.text.map((t, k) => (
            <p key={k}>{t}</p>
          ))}
