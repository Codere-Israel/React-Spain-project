    ...
    inner: [
      {
        ...
        liText:
          "Los apostadores eligen al equipo o jugador que creen que ganará el enfrentamiento.",
      },
      {
        liBold: "Apuestas al Handicap",
        liText:
          "Estas ajustan las probabilidades al dar ventaja o desventaja a un equipo para equilibrar las posibilidades de apuesta.",
      },
      {
        liBold: "Apuestas al Resultado Correcto",
        liText:
          "Los apostadores apuestan por un resultado específico en un juego o serie de juegos.",
      },
      {
        liBold: "Apuestas a los Totales",
        liText:
          "Se centran en aspectos como el número total de rondas, puntos o goles marcados en una partida.",
      },
      {
      ...
      },
      {
        ...
        liText:
          "en eSports implican elegir al equipo o jugador que se prevé ganará el torneo por completo, siendo una de las apuestas más emocionantes.",
      },
      {
        liBold: "Apuesta Ganador de Mapa ",
        liText:
          "implican predecir qué equipo o jugador ganará un mapa específico en una partida o serie de eSports. Esto añade emoción al permitir apostar en mapas individuales.",
      },
      {
        liBold: "Handicap (por Ronda)",
        liText:
          "dan ventaja o desventaja a un equipo antes de una partida o ronda, comunes en juegos de disparos en primera persona como Counter-Strike: Global Offensive.",
      },
      {
        liBold: "Ganador de Ronda",
        liText:
          "se centran en predecir qué equipo ganará una ronda específica, considerando el conocimiento de los equipos, su rendimiento previo, estrategias y condiciones del juego.",
      },
    ],
  };
      ...
      <Container>
        <div className="event">
          <Container>
            ...
            <div className="mt-4" id={topDeJuegos.id}>
              <h2>{topDeJuegos.h2}</h2>
              <p>{topDeJuegos.p1}</p>
              <p>{topDeJuegos.p2}</p>
            </div>
            <div className="mt-4" id={losMejoresEquipos.id}>
              <h2>{losMejoresEquipos.h2}</h2>
              <p>{losMejoresEquipos.p1}</p>
              <p>{losMejoresEquipos.p2}</p>
            </div>

            <div className="mt-4" id={principalesTorneos.id}>
            ...
            <div id={tiposDeApuestasEnEsports.id} className="mt-4">
              <h2>{tiposDeApuestasEnEsports.h2}</h2>
              <p>{tiposDeApuestasEnEsports.p}</p>
              <ul>
                {tiposDeApuestasEnEsports.inner.map((m, k) => (
                  <li key={k}>
                    <u>{m.liBold}</u>:{m.liText}
                  </li>
                ))}
              </ul>
            </div>

            <div className="mt-4" id={preguntasFrecuentes.id}>
